    ...
    def loader():
        ...
        for count in range(num_batches):
            batch = {
                key: [
                    data_dict[key][ix]
                    for ix in indices[count * batch_size : (count + 1) * batch_size]
                ]
                for key in keys
            }
            yield batch
        ...
        if remainder:
            batch = {
                key: [data_dict[key][ix] for ix in indices[num_batches * batch_size :]]
                for key in keys
            }
            yield batch
...
def convert_to_tensors(text_indices: "list[list[int]]") -> torch.Tensor:
    ...
    tensor = torch.zeros((len(text_indices), max_length), dtype=torch.int32)
    for i, text in enumerate(text_indices):
        tensor[i, : len(text)] = torch.tensor(text, dtype=torch.int32)
...
class PooledLogisticRegression(nn.Module):
    def __init__(self, embedding: nn.Embedding):
        ...
        self.layer_pred = nn.Linear(
            2 * self.embedding.embedding_dim, 1
        )  # (2 * hidden_size, 1)
    ...
    def forward(self, premise: torch.Tensor, hypothesis: torch.Tensor) -> torch.Tensor:
        ...
        concat = torch.cat(
            (premise_pool, hypothesis_pool), dim=1
...
def eval_run(
    ...
    for batch in loader():
        # Get the label
        label = torch.tensor(batch["label"], dtype=torch.float).to(device)
        # Get the prediction
...
class DeepNeuralNetwork(nn.Module):
    def __init__(self, embedding: nn.Embedding, hidden_size: int, num_layers: int = 2):
        ...
        self.ff_layers = nn.ModuleList(
            [nn.Linear(2 * self.embedding.embedding_dim, hidden_size)]
